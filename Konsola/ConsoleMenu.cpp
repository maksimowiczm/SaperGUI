...
namespace konsola
{
	ConsoleMenu::ConsoleMenu(const int16_t cols, const int16_t rows) : cols_(cols), rows_(rows)
	{
		menu_ = {
			{
	...
	void ConsoleMenu::Display(const bool clear) const
	{
		Print("saper", {static_cast<int16_t>(cols_ / 2), 0}, NULL, clear);
	...
	bool ConsoleMenu::checkIfControlUsed(controls control, int ch, std::vector<std::tuple<controls, std::vector<int>>> controls)
	{
	...
	bool ConsoleMenu::checkIfControlUsed(controls control, const int ch) const
	{
		return checkIfControlUsed(control, ch, controls_);
	}

	int ConsoleMenu::getKey()
	{
		auto ch = _getch();
		if (ch == 224)
			ch += _getch();
	...
	std::vector<std::vector<bool>> ConsoleMenu::StringToBlocks(const std::string& str) const
	{
	...
	void ConsoleMenu::Print(const std::string& str, const COORD startingPos, WORD consoleAtribute, bool clear) const
	{
		...
		SetConsoleCursorPosition(hConsole, {static_cast<int16_t>(startingPos.X - blocks.front().size() / 2), startingPos.Y});
		...
		for (const auto& row : blocks)
		{
			...
			SetConsoleCursorPosition(hConsole, {
				                         static_cast<int16_t>(startingPos.X - blocks.front().size() / 2), static_cast<int16_t>(startingPos.Y + Y++)
			                         });
		}
	}

	void ConsoleMenu::Use() const
	{
		std::get<2>(activeMenu_[cursor_])();
	}

	void ConsoleMenu::StartGame(std::tuple<int16_t, int16_t, int16_t> level)
	{
		auto& [x, y, mines] = level;
		auto game = ConsoleGame(x, y, mines, COORD{static_cast<int16_t>(cols_ / 2), 0}, controls_);
	...
	void ConsoleMenu::PrintGameOver(const bool clear, const bool win, const int time)
	{
		const std::string result = win ? "rozbrojone" : "game over";
		Print(result, {static_cast<int16_t>(cols_ / 2), 6}, NULL, clear);
		if (win)
			Print("twoj czas " + std::to_string(time), {static_cast<int16_t>(cols_ / 2), 12}, NULL, clear);

		activeMenu_ = gameOverMenu_;
		cursor_ = 0;
	}

	std::tuple<int16_t, int16_t, int16_t> ConsoleMenu::CustomLevel()
	{
